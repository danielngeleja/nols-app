...
router.post(
  ...
  asyncHandler(async (req: any, res) => {
    ...
    const item = await prisma.tourCase.findFirst({ where: { id: caseId, tourBookingId: bookingId, booking: { operatorAgentId: gate.agent.id } } });
...
router.post(
  ...
  asyncHandler(async (req: any, res) => {
    ...
    if (!items.length || items.some((item: any) => !["NON_REFUNDABLE_COMPONENT", "CONSUMED_SERVICE", "RECOVERY_COST"].includes(item.kind) || !item.description || !(item.amount > 0) || !/^https?:\/\//i.test(item.evidenceUrl))) {
