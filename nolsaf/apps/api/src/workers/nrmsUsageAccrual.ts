...
export async function runNrmsUsageAccrual(now = new Date()) {
  ...
  if (!reservations.length) return { properties: 0, usageEvents: 0 };
  ...
  for (const [propertyId, propertyReservations] of byProperty) {
    try {
      ...
      // One property's failure (e.g. a transient lock) must not stop the rest
      // of the sweep - it will simply be picked up again on the next run.
      console.error(`[nrms-usage-accrual] property ${propertyId} failed`, error);
    }
  }
  return { properties: propertiesProcessed, usageEvents: totalEvents };
}
