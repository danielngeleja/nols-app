    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-neutral-50 p-6" style={GUEST_FONT_STYLE}>
        <div className="w-full max-w-sm rounded-3xl border border-neutral-200 bg-white p-6 text-center shadow-sm">
          ...
          <p className="m-0 text-[15px] font-bold text-neutral-900">{loadError || "This QR code is not valid."}</p>
          <p className="mb-0 mt-2 text-[13px] text-neutral-400">Please ask a staff member for help.</p>
        </div>
      </div>
    );
  }
        ...
        <header className="relative mx-2 mt-2 overflow-hidden rounded-[14px] bg-[#073f35] text-white shadow-[0_14px_38px_rgba(7,63,53,0.14)] sm:mx-4 sm:mt-4 sm:rounded-[18px]">
          ...
          <div className="relative mx-auto max-w-5xl px-5 pb-10 pt-5 sm:px-8 sm:pb-12 sm:pt-7">
            <div className="flex items-center justify-between gap-4">
              <p className="m-0 text-[12px] font-bold uppercase tracking-[0.2em] text-emerald-200/80">{menu.property.title}</p>
              {!cancelled && (
                <span className="inline-flex shrink-0 items-center gap-2 rounded-lg bg-white/10 px-3 py-2 text-[12px] font-bold text-emerald-50 ring-1 ring-white/10">
            ...
            <div className="mt-5 flex items-start gap-3">
              ...
              <div className="min-w-0">
                <p className="m-0 text-[12px] font-bold uppercase tracking-[0.16em] text-emerald-200/75">Guest order tracking</p>
                ...
                <p className="mb-0 mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-[13px] font-semibold text-emerald-100/70 sm:text-[15px]">
        ...
        <main className="relative mx-auto -mt-5 max-w-5xl px-4 sm:-mt-6 sm:px-6">
          ...
          <section className="min-w-0 overflow-hidden rounded-[16px] border border-stone-200 bg-white shadow-[0_12px_30px_rgba(28,25,23,0.07)]" aria-label="Order progress">
            {cancelled ? (
              <div className="px-6 py-12 text-center">
                ...
                <p className="mx-auto mb-0 mt-2 max-w-sm text-[15px] leading-6 text-neutral-500">Please speak with a staff member if this was unexpected.</p>
              </div>
            ) : (
              <>
                <div className="border-b border-stone-100 bg-[linear-gradient(135deg,#ecfdf5_0%,#ffffff_72%)] px-5 py-5 sm:px-6">
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <p className="m-0 text-[12px] font-bold uppercase tracking-[0.18em] text-emerald-700">Live progress</p>
                      <h2 className="mb-0 mt-1 text-xl font-bold tracking-[-0.02em] text-neutral-950 sm:text-2xl">{activeStep?.label}</h2>
                      <p className="mb-0 mt-1.5 text-[15px] leading-6 text-neutral-500">{statusMessage}</p>
                    </div>
                    <span className="inline-flex shrink-0 rounded-lg bg-emerald-100 px-2.5 py-1.5 text-[12px] font-bold text-emerald-800">Step {Math.max(stepIndex + 1, 1)} of 6</span>
                  </div>
                </div>
                      ...
                      <div className="min-w-0 flex-1 pt-2">
                        <div className="flex min-w-0 items-start justify-between gap-3">
                          <div className="flex min-w-0 items-center gap-2">
                            <p className={`m-0 text-[15px] font-bold sm:text-base ${reached ? "text-neutral-900" : "text-neutral-400"}`}>{step.label}</p>
                            {current && <span className="shrink-0 text-[11px] font-bold uppercase tracking-wider text-emerald-700">Now</span>}
                          </div>
                          {timestamp && (
                            <div className="flex shrink-0 items-center gap-1.5 text-right">
                              {elapsed && <span className="rounded-md bg-emerald-50 px-1.5 py-1 text-[11px] font-bold text-emerald-700">+{elapsed}</span>}
                              <time dateTime={trackingTimes[index] ?? undefined} className="text-[12.5px] font-bold tabular-nums text-neutral-500">{timestamp}</time>
                            </div>
                          )}
                        </div>
                        {current && <p className="mb-0 mt-1 text-[13px] leading-5 text-neutral-500">{statusMessage}</p>}
                        {index === 5 && !reached && (
                          <p className={`mb-0 mt-1 text-[13px] font-semibold leading-5 ${order.settlementMode === "ROOM_FOLIO" ? "text-blue-600" : "text-amber-700"}`}>
                ...
                <p className="mb-0 border-t border-stone-100 px-5 py-3 text-[12px] text-neutral-400 sm:px-6">Stage times and elapsed service intervals are recorded automatically.</p>
              </>
            )}
          </section>
          ...
          <section className="min-w-0 rounded-[16px] border border-stone-200 bg-white p-5 shadow-[0_12px_30px_rgba(28,25,23,0.06)] sm:p-6" aria-label="Order summary">
            <div className="flex items-center gap-3 border-b border-stone-100 pb-4">
              ...
              <div className="min-w-0"><p className="m-0 text-[11px] font-bold uppercase tracking-[0.16em] text-emerald-700">Order summary</p><h2 className="mb-0 mt-0.5 truncate text-lg font-bold text-neutral-950">{order.outlet?.name}</h2></div>
            </div>
            <ul className="m-0 list-none space-y-0 p-0 pt-2">
              {order.items.map((item, index) => (
                <li key={index} className="flex min-w-0 items-center justify-between gap-3 border-b border-stone-100 py-3 last:border-b-0">
                  <span className="min-w-0 text-[15px] font-semibold text-neutral-700"><span className="mr-2 inline-flex h-7 min-w-7 items-center justify-center rounded-md bg-stone-100 px-1.5 text-[13px] font-bold text-neutral-600">{item.quantity}</span>{item.name}</span>
                  <span className="shrink-0 text-[15px] font-bold text-neutral-900">{money(item.lineTotal, order.currency)}</span>
                </li>
              ))}
            </ul>
            {order.note && <div className="mt-3 flex items-start gap-2.5 rounded-lg bg-stone-50 px-3 py-2.5 text-[13px] leading-5 text-neutral-500"><ReceiptText className="mt-0.5 h-3.5 w-3.5 shrink-0 text-emerald-700" /><span><strong className="text-neutral-700">Serving note:</strong> {order.note}</span></div>}
            ...
            {!cancelled && paymentConfirmed && (
              <div className="mt-4 flex items-start gap-2.5 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-3 text-[13px] leading-5 text-emerald-800"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0" /><span><strong>Paid and confirmed by the attendant.</strong>{order.settlementMethod ? ` Payment method: ${order.settlementMethod.replaceAll("_", " ").toLowerCase()}.` : ""}</span></div>
            )}
            {!cancelled && postedToRoom && (
              <div className="mt-4 flex items-start gap-2.5 rounded-lg border border-blue-200 bg-blue-50 px-3 py-3 text-[13px] leading-5 text-blue-800"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0" /><span><strong>Added to your room bill.</strong> The amount will appear on your folio at checkout.</span></div>
            )}
            {!cancelled && !done && order.settlementMode === "ROOM_FOLIO" && (
              <div className="mt-4 flex items-start gap-2.5 rounded-lg border border-blue-200 bg-blue-50 px-3 py-3 text-[13px] leading-5 text-blue-800"><ReceiptText className="mt-0.5 h-4 w-4 shrink-0" /><span><strong>Room bill pending.</strong> This amount will be posted to your folio after service.</span></div>
            )}
            {!cancelled && !done && order.settlementMode !== "ROOM_FOLIO" && (
              <div className="mt-4 flex items-start gap-2.5 rounded-lg border border-amber-200 bg-amber-50 px-3 py-3 text-[13px] leading-5 text-amber-800"><ReceiptText className="mt-0.5 h-4 w-4 shrink-0" /><span><strong>{paymentMethodLabel(order.guestPaymentMethod)} selected · Not paid yet · {money(order.total, order.currency)} due.</strong> The attendant will confirm the method actually received when settling your order.</span></div>
            )}
          </section>
              ...
              <ul className="relative m-0 mt-4 list-none space-y-2.5 p-0">
                {selectedPayInstructions.map((row, index) => (
                  <li key={index} className="rounded-lg border border-white/10 bg-white/10 px-3.5 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] backdrop-blur-sm">
                    <p className="m-0 text-[11px] font-bold uppercase tracking-[0.14em] text-emerald-100/70">{row.label}</p>
                    <p className="mb-0 mt-1 text-base font-bold tracking-wide text-white">{row.value}</p>
                    {row.name && <p className="mb-0 mt-1 text-[13px] text-emerald-50/70">{row.name}</p>}
                  </li>
                ))}
              </ul>
              <p className="relative mb-0 mt-3 text-[13px] leading-5 text-emerald-50/70">
            ...
            <section className="relative mt-4 min-w-0 overflow-hidden rounded-[16px] border border-amber-200/80 bg-[linear-gradient(135deg,#fffbeb_0%,#ffffff_50%,#ecfdf5_100%)] p-5 shadow-[0_12px_30px_rgba(120,83,25,0.07)] sm:p-6" aria-label="Service feedback">
              ...
              {order.feedbackAt ? (
                <div className="relative flex flex-col items-center py-3 text-center">
                  ...
                  <p className="mx-auto mb-0 mt-2 max-w-lg text-[15px] leading-6 text-neutral-500">Your rating has been shared with the property team.</p>
                  {order.tipIntent === "INTERESTED" && order.tipSuggestedAmount != null && (
                    <p className="mb-0 mt-2 rounded-lg bg-white/80 px-3 py-2 text-[13px] font-semibold text-emerald-800 ring-1 ring-emerald-100">Tip preference noted: {money(order.tipSuggestedAmount, order.currency)}. This was not charged automatically.</p>
                  )}
                </div>
              ) : (
                <div className="relative grid min-w-0 gap-6 lg:grid-cols-[1.08fr_0.92fr] lg:items-start">
                  <div className="min-w-0">
                    <div className="flex items-start gap-3"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-amber-100 text-amber-700 ring-1 ring-amber-200"><Heart className="h-5 w-5" /></span><div><p className="m-0 text-[12px] font-bold uppercase tracking-[0.16em] text-amber-700">A quick thank-you</p><h2 className="mb-0 mt-1 text-xl font-bold tracking-[-0.02em] text-neutral-950">How was your service?</h2><p className="mb-0 mt-1 text-[15px] leading-6 text-neutral-500">Your feedback helps the team make the next order even better.</p></div></div>
                    ...
                    <div className="mt-4 flex flex-wrap items-center gap-2" role="radiogroup" aria-label="Service rating">
                      ...
                      {feedbackRating > 0 && <span className="ml-1 text-[13px] font-bold text-neutral-600">{["", "Needs attention", "Fair", "Good", "Very good", "Excellent"][feedbackRating]}</span>}
                    </div>

                    <label className="mt-4 block min-w-0"><span className="mb-1.5 block text-[13px] font-bold text-neutral-600">Anything you would like the team to know? <span className="font-normal text-neutral-400">(optional)</span></span><textarea value={feedbackComment} onChange={(event) => setFeedbackComment(event.target.value.slice(0, 500))} rows={2} placeholder="A short note about your experience" className="box-border w-full max-w-full resize-none rounded-lg border border-stone-200 bg-white/80 px-3 py-3 text-[15px] leading-6 text-neutral-900 outline-none placeholder:text-neutral-400 focus:border-emerald-400 focus:ring-4 focus:ring-emerald-500/10" style={{ fontFamily: "inherit" }} /></label>
                  </div>

                  <div className="min-w-0 rounded-xl border border-white bg-white/75 p-4 shadow-sm backdrop-blur-sm">
                    <p className="m-0 text-[12px] font-bold uppercase tracking-[0.16em] text-emerald-700">Entirely optional</p>
                    <h3 className="mb-0 mt-1 text-base font-bold text-neutral-950">Would you like to thank the team with a tip?</h3>
                    <p className="mb-0 mt-1 text-[13px] leading-5 text-neutral-500">Choose only if you wish. A tip is never required.</p>
                    <div className="mt-3 grid grid-cols-4 gap-2" role="radiogroup" aria-label="Tip preference">
                      {[0, 5, 10, 15].map((percent) => (
                        <button key={percent} type="button" role="radio" aria-checked={feedbackTipPercent === percent} onClick={() => setFeedbackTipPercent(percent)} className={`min-h-10 rounded-lg border px-2 text-[13px] font-bold transition ${feedbackTipPercent === percent ? "border-emerald-600 bg-emerald-700 text-white" : "border-stone-200 bg-white text-neutral-600 hover:border-emerald-200"}`}>{percent === 0 ? "No tip" : `${percent}%`}</button>
                      ))}
                    </div>
                    {selectedTipAmount != null && (
                      <div className="mt-3 flex items-center justify-between rounded-lg bg-emerald-50 px-3 py-2.5 text-[13px]"><span className="font-semibold text-emerald-700">Suggested tip</span><strong className="text-[15px] text-emerald-900">{money(selectedTipAmount, order.currency)}</strong></div>
                    )}
                    <p className="mb-0 mt-3 text-[12px] leading-4 text-neutral-400">Selecting a tip records your preference only. It does not charge your room or payment method automatically; please give it directly to the property team.</p>

                    {feedbackError && <div className="mt-3 flex items-start gap-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2.5 text-[13px] text-red-700"><AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0" />{feedbackError}</div>}
                    <button type="button" onClick={() => void submitFeedback()} disabled={feedbackRating < 1 || feedbackSubmitting} className="mt-4 flex min-h-12 w-full items-center justify-center gap-2 rounded-lg border-0 bg-[#073f35] px-4 py-3 text-[15px] font-bold text-white shadow-[0_8px_20px_rgba(7,63,53,0.18)] transition hover:bg-emerald-900 disabled:cursor-not-allowed disabled:bg-stone-200 disabled:text-stone-400 disabled:shadow-none">{feedbackSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Heart className="h-4 w-4" />}{feedbackSubmitting ? "Sending…" : "Send thanks & feedback"}</button>
                  </div>
                </div>
              )}
            </section>
          ...
          <button
            ...
            className="mx-auto mt-5 flex min-h-12 w-full max-w-sm items-center justify-center gap-2 rounded-lg border border-stone-200 bg-white px-4 py-3 text-[15px] font-bold text-neutral-700 shadow-sm transition hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-900"
          >
            <ChevronLeft className="h-4 w-4" /> Back to menu
          </button>

          <p className="mb-0 mt-7 text-center text-[12px] text-neutral-400">
    ...
    <div className="min-h-screen overflow-x-hidden bg-[#f6f4ef] pb-28 text-neutral-950 lg:pb-14" style={GUEST_FONT_STYLE}>
      <header className="relative mx-2 mt-2 overflow-hidden rounded-[14px] bg-[#073f35] text-white shadow-[0_14px_38px_rgba(7,63,53,0.14)] sm:mx-4 sm:mt-4 sm:rounded-[18px]">
        ...
        <div className="mx-auto max-w-6xl px-5 pb-6 pt-5 sm:px-8 sm:pb-7 sm:pt-6">
          <div className="flex items-center justify-between gap-4">
            <div className="inline-flex items-center gap-2 text-[12px] font-bold uppercase tracking-[0.2em] text-emerald-200/80"><Sparkles className="h-3.5 w-3.5" /> Guest dining</div>
            <span className="inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-[12px] font-semibold text-emerald-50 backdrop-blur"><MapPin className="h-3 w-3" /> {pointLabel}</span>
          </div>
          <div className="mt-5 grid items-center gap-5 sm:mt-6 lg:grid-cols-[minmax(0,1fr)_32rem] lg:gap-10">
            <div className="max-w-2xl">
              <p className="m-0 text-[12px] font-bold uppercase tracking-[0.24em] text-emerald-200/70">{menu.property.title}</p>
              <h1 className="mb-0 mt-1.5 text-2xl font-bold tracking-[-0.03em] text-white sm:text-3xl">A menu made for your stay</h1>
              <p className="mb-0 mt-2 max-w-xl text-[13px] leading-5 text-emerald-50/70 sm:text-[15px]">Choose from today&apos;s live selection and send your order directly to the property team.</p>
              <div className="mt-3.5 flex flex-wrap items-center gap-x-4 gap-y-1.5 text-[11px] font-semibold text-emerald-100/65"><span className="inline-flex items-center gap-1.5"><span className="h-1.5 w-1.5 rounded-full bg-emerald-300" /> Live availability</span><span className="inline-flex items-center gap-1.5"><span className="h-1.5 w-1.5 rounded-full bg-emerald-300" /> Direct to the team</span><span className="inline-flex items-center gap-1.5"><span className="h-1.5 w-1.5 rounded-full bg-emerald-300" /> Live tracking</span></div>
            </div>
            ...
            <aside className="rounded-xl border border-white/10 bg-white/[0.065] p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.07)] backdrop-blur-sm" aria-label="How ordering works">
              <div className="mb-2.5 flex items-center justify-between gap-3"><div><p className="m-0 text-[11px] font-bold uppercase tracking-[0.16em] text-emerald-200/65">From menu to service</p><h2 className="mb-0 mt-0.5 text-[13px] font-bold text-white">Four simple steps</h2></div><span className="text-[11px] font-bold uppercase tracking-[0.12em] text-emerald-100/55">How it works</span></div>
              <ol className="m-0 grid list-none grid-cols-4 gap-1.5 p-0">
                <li className="min-w-0 rounded-lg border border-white/10 bg-black/10 p-2"><div className="flex items-center justify-between gap-1"><span className="flex h-7 w-7 items-center justify-center rounded-lg bg-emerald-300/15 text-emerald-100"><Search className="h-3 w-3" /></span><span className="text-[11px] font-bold text-emerald-200/40">01</span></div><p className="mb-0 mt-1.5 truncate text-[11px] font-bold text-white">Browse</p></li>
                <li className="min-w-0 rounded-lg border border-white/10 bg-black/10 p-2"><div className="flex items-center justify-between gap-1"><span className="flex h-7 w-7 items-center justify-center rounded-lg bg-emerald-300/15 text-emerald-100"><ShoppingBasket className="h-3 w-3" /></span><span className="text-[11px] font-bold text-emerald-200/40">02</span></div><p className="mb-0 mt-1.5 truncate text-[11px] font-bold text-white">Choose</p></li>
                <li className="min-w-0 rounded-lg border border-white/10 bg-black/10 p-2"><div className="flex items-center justify-between gap-1"><span className="flex h-7 w-7 items-center justify-center rounded-lg bg-emerald-300/15 text-emerald-100"><ChefHat className="h-3 w-3" /></span><span className="text-[11px] font-bold text-emerald-200/40">03</span></div><p className="mb-0 mt-1.5 truncate text-[11px] font-bold text-white">Send</p></li>
                <li className="min-w-0 rounded-lg border border-white/10 bg-black/10 p-2"><div className="flex items-center justify-between gap-1"><span className="flex h-7 w-7 items-center justify-center rounded-lg bg-emerald-300/15 text-emerald-100"><CheckCircle2 className="h-3 w-3" /></span><span className="text-[11px] font-bold text-emerald-200/40">04</span></div><p className="mb-0 mt-1.5 truncate text-[11px] font-bold text-white">Track</p></li>
              </ol>
            </aside>
          </div>
        </div>
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 sm:px-8 sm:py-8">
          ...
          <>
            ...
            {!isBarOutlet && categories.length > 1 && !searchQuery && (
              <nav className="-mx-4 flex gap-2 overflow-x-auto px-4 py-4 sm:mx-0 sm:px-0" aria-label="Menu categories">
                {categories.map((category, index) => <button key={category.name} type="button" onClick={() => jumpToCategory(category.name, index)} className={`shrink-0 rounded-full border px-3.5 py-2 text-[12.5px] font-bold transition ${selectedCategory === category.name ? "border-emerald-700 bg-emerald-700 text-white" : "border-stone-200 bg-white text-neutral-600 hover:border-emerald-200 hover:text-emerald-800"}`}>{category.name}<span className="ml-1.5 text-[11px] opacity-60">{category.items.length}</span></button>)}
              </nav>
            )}
            ...
            <div className="mt-2 grid items-start gap-7 xl:grid-cols-[minmax(0,1fr)_19rem]">
              <div className="min-w-0 space-y-9">
                {menuSections.length === 0 ? (
                  <div className="rounded-[24px] border border-dashed border-stone-300 bg-white px-6 py-14 text-center"><Search className="mx-auto h-7 w-7 text-stone-300" /><p className="mb-0 mt-3 text-[15px] font-bold text-neutral-700">Nothing matched “{searchQuery}”</p><button type="button" onClick={() => setSearchQuery("")} className="mt-3 rounded-full border border-stone-200 bg-white px-4 py-2 text-[13px] font-bold text-emerald-800 hover:bg-emerald-50">Clear search</button></div>
                ) : isBarOutlet ? (
                  ...
                  <section key={category.name} id={`menu-category-${categoryIndex}`} className="scroll-mt-36">
                    <div className="mb-3 flex items-baseline justify-between gap-3"><h3 className="m-0 text-xl font-bold tracking-[-0.02em] text-neutral-900">{category.name}</h3><span className="text-[12px] font-bold uppercase tracking-[0.12em] text-neutral-400">{category.items.length} {category.items.length === 1 ? "item" : "items"}</span></div>
                        ...
                        return (
                          <article key={item.id} className={`group relative flex min-h-36 overflow-hidden rounded-[20px] border bg-white shadow-[0_8px_28px_rgba(38,35,28,0.05)] transition ${unavailable ? "border-stone-200 bg-stone-50/80" : "border-stone-200 hover:-translate-y-0.5 hover:border-emerald-200 hover:shadow-[0_14px_35px_rgba(6,78,59,0.09)]"}`}>
                            <div className="min-w-0 flex-1 p-4">
                              <p className={`m-0 pr-1 text-[15px] font-bold leading-snug ${unavailable ? "text-neutral-500" : "text-neutral-950"}`}>{item.name}</p>
                              {item.description && <p className="mb-0 mt-1.5 line-clamp-2 text-[12.5px] leading-[1.55] text-neutral-500">{item.description}</p>}
                              <div className="mt-3 flex items-end justify-between gap-2">
                                <p className={`m-0 text-[15px] font-bold ${unavailable ? "text-neutral-400" : "text-emerald-800"}`}>{money(item.price, activeOutlet.currency)}</p>
                                {unavailable ? <span className="rounded-full bg-stone-200/70 px-2.5 py-1 text-[11px] font-bold text-stone-500">Unavailable</span> : qty === 0 ? <button type="button" disabled={!orderingEnabled} onClick={() => changeQty(item.id, 1)} aria-label={orderingEnabled ? `Add ${item.name}` : `${item.name}, order by scanning your table or room QR code`} className="inline-flex h-8 items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-3 text-[12px] font-bold text-emerald-800 transition hover:bg-emerald-100 active:scale-95 disabled:cursor-not-allowed disabled:border-stone-200 disabled:bg-stone-100 disabled:text-stone-400 disabled:hover:bg-stone-100"><Plus className="h-3.5 w-3.5" /> Add</button> : <span className="inline-flex h-8 items-center rounded-full bg-[#073f35] text-white shadow-sm"><button type="button" onClick={() => changeQty(item.id, -1)} aria-label={`Remove one ${item.name}`} className="flex h-8 w-8 items-center justify-center rounded-full border-0 bg-transparent text-white"><Minus className="h-3 w-3" /></button><span className="min-w-5 text-center text-[12.5px] font-bold">{qty}</span><button type="button" onClick={() => changeQty(item.id, 1)} aria-label={`Add one ${item.name}`} className="flex h-8 w-8 items-center justify-center rounded-full border-0 bg-transparent text-white"><Plus className="h-3 w-3" /></button></span>}
                              </div>
                            </div>
              ...
              <aside className="sticky top-24 hidden overflow-hidden rounded-[24px] border border-stone-200 bg-white shadow-[0_16px_45px_rgba(38,35,28,0.08)] xl:block">
                <div className="border-b border-stone-100 px-5 py-4"><div className="flex items-center gap-2"><ReceiptText className="h-4 w-4 text-emerald-700" /><h3 className="m-0 text-[15px] font-bold text-neutral-900">Your order</h3></div><p className="mb-0 mt-1 text-[12px] text-neutral-400">{pointLabel} · {activeOutlet.name}</p></div>
                {cartCount === 0 ? <div className="px-5 py-6 text-center"><span className="mx-auto flex h-11 w-11 items-center justify-center rounded-full bg-stone-100 text-stone-400"><ShoppingBasket className="h-5 w-5" /></span><p className="mb-0 mt-3 text-[13px] font-bold text-neutral-600">Your basket is ready</p><p className="mb-0 mt-1 text-[12px] leading-4 text-neutral-400">Add a selection to begin your order.</p></div> : <div className="p-5"><div className="max-h-64 space-y-3 overflow-y-auto pr-1">{cartLines.map(({ item, quantity }) => <div key={item.id} className="flex items-center gap-2.5"><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-emerald-50 text-[12px] font-bold text-emerald-800">{quantity}</span><p className="m-0 min-w-0 flex-1 truncate text-[13px] font-semibold text-neutral-700">{item.name}</p><span className="shrink-0 text-[12px] font-bold text-neutral-600">{money(item.price * quantity, activeOutlet.currency)}</span></div>)}</div><div className="mt-4 flex items-baseline justify-between border-t border-stone-100 pt-4"><span className="text-[13px] font-bold text-neutral-600">Total</span><span className="text-base font-bold text-emerald-800">{money(cartTotal, activeOutlet.currency)}</span></div><button type="button" onClick={() => setCartOpen(true)} className="mt-4 flex h-11 w-full items-center justify-center gap-2 rounded-xl border-0 bg-[#073f35] px-4 text-[13px] font-bold text-white shadow-sm transition hover:bg-emerald-900">Review order <ArrowRight className="h-3.5 w-3.5" /></button></div>}
                {recentOrders.length > 0 && (
                  <div className="border-t border-stone-100 px-5 py-4">
                    <div className="mb-3 flex items-center justify-between gap-3"><div className="flex items-center gap-2"><History className="h-3.5 w-3.5 text-emerald-700" /><p className="m-0 text-[13px] font-bold text-neutral-800">Recent orders</p></div><span className="text-[11px] font-semibold text-neutral-400">This device</span></div>
                    <div className="space-y-2">
                      {recentOrders.slice(0, 3).map((entry) => (
                        <button key={entry.code} type="button" onClick={() => openRecentOrder(entry)} className="flex w-full min-w-0 items-center gap-3 rounded-lg border border-stone-200 bg-stone-50/70 px-3 py-2.5 text-left transition hover:border-emerald-200 hover:bg-emerald-50">
                          ...
                          <span className="min-w-0 flex-1"><span className="block truncate text-[12.5px] font-bold text-neutral-800">{entry.order.orderNumber}</span><span className="mt-0.5 block truncate text-[11px] text-neutral-500">{guestStatusLabel(entry.order.status)} · {shortOrderDate(entry.order.placedAt)}</span></span>
                          <span className="shrink-0 text-[12px] font-bold text-emerald-800">{money(entry.order.total, entry.order.currency)}</span>
                          <ArrowRight className="h-3.5 w-3.5 shrink-0 text-neutral-400" />
                        </button>
                      ))}
                    </div>
                    <p className="mb-0 mt-2 text-[11px] leading-4 text-neutral-400">Receipts remain available on this device after service is complete.</p>
                  </div>
                )}
                <div className="flex items-start gap-2 border-t border-stone-100 bg-stone-50/70 px-5 py-3 text-[11px] leading-4 text-stone-500"><ShieldCheck className="mt-0.5 h-3 w-3 shrink-0 text-emerald-700" /> Your order goes directly to the property team.</div>
              </aside>
                  ...
                  <div className="divide-y divide-stone-100">
                    {recentOrders.slice(0, 3).map((entry) => (
                      <button key={entry.code} type="button" onClick={() => openRecentOrder(entry)} className="flex w-full min-w-0 items-center gap-3 border-0 bg-white px-4 py-3.5 text-left transition hover:bg-emerald-50">
                        ...
                        <span className="min-w-0 flex-1"><span className="block truncate text-[15px] font-bold text-neutral-800">{entry.order.orderNumber}</span><span className="mt-0.5 block truncate text-[12.5px] text-neutral-500">{guestStatusLabel(entry.order.status)} · {shortOrderDate(entry.order.placedAt)}</span></span>
                        <span className="shrink-0 text-[13px] font-bold text-emerald-800">{money(entry.order.total, entry.order.currency)}</span>
        ...
        <p className="mb-0 mt-14 text-center text-[12px] text-stone-400">Guest ordering powered by <span className="font-bold text-emerald-800">NoLSAF</span></p>
      </main>
        ...
        <>
          <div className="fixed right-3 z-[60] sm:hidden" style={{ bottom: "calc(5.5rem + env(safe-area-inset-bottom))" }}>
            <button type="button" onClick={() => setCartOpen(true)} aria-label={`View order with ${cartCount} item${cartCount === 1 ? "" : "s"}`} className="relative flex h-14 w-14 items-center justify-center rounded-xl border border-white/15 bg-[#073f35] text-white shadow-[0_14px_32px_rgba(7,63,53,0.4)] transition active:scale-95">
              <ShoppingBasket className="h-5 w-5" /><span className="absolute -right-1.5 -top-1.5 flex h-6 min-w-6 items-center justify-center rounded-full border-2 border-[#f6f4ef] bg-emerald-400 px-1 text-[12px] font-bold text-emerald-950">{cartCount}</span>
            </button>
          </div>
          <div className="fixed bottom-5 right-4 z-[60] hidden sm:block xl:hidden">
            <button type="button" onClick={() => setCartOpen(true)} aria-label={`View order with ${cartCount} item${cartCount === 1 ? "" : "s"}`} className="flex h-14 items-center gap-3 rounded-xl border border-white/10 bg-[#073f35] px-3.5 text-white shadow-[0_14px_36px_rgba(7,63,53,0.36)] transition active:scale-[0.98]">
              <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-white/12"><ShoppingBasket className="h-4 w-4" /></span><span className="min-w-0 text-left"><span className="block text-[12.5px] font-bold">View order</span><span className="mt-0.5 block whitespace-nowrap text-[11px] font-semibold text-emerald-100/70">{money(cartTotal, activeOutlet?.currency ?? "TZS")} · {cartCount} item{cartCount === 1 ? "" : "s"}</span></span><ArrowRight className="h-4 w-4 shrink-0 text-emerald-200" />
            </button>
          </div>
        </>
      )}
        ...
        <div className="fixed inset-0 z-[300] flex items-center justify-center overflow-x-hidden p-2 sm:p-4 lg:p-6">
          ...
          <div role="dialog" aria-modal="true" aria-labelledby="guest-order-title" className="relative box-border max-h-[calc(100dvh-1.5rem)] w-[calc(100%-1.5rem)] max-w-lg min-w-0 overscroll-contain overflow-x-hidden overflow-y-auto rounded-[14px] border border-stone-300 bg-white p-5 pb-[calc(1.75rem+env(safe-area-inset-bottom))] shadow-[0_28px_80px_rgba(4,31,26,0.34)] sm:max-h-[calc(100dvh-2.5rem)] lg:max-h-[90dvh] lg:p-6">
            ...
            <div className="-mx-5 -mt-5 border-b border-stone-100 bg-gradient-to-r from-emerald-50/70 via-white to-white px-5 pb-4 pt-5 lg:-mx-6 lg:-mt-6 lg:px-6 lg:pt-6">
              <div className="flex min-w-0 items-start gap-3 pr-10"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-white text-emerald-800 shadow-sm ring-1 ring-emerald-100">{isBarOutlet ? <Wine className="h-5 w-5" /> : <UtensilsCrossed className="h-5 w-5" />}</span><div className="min-w-0"><p className="m-0 text-[12px] font-bold uppercase tracking-[0.16em] text-emerald-700">{isBarOutlet ? "Review bar order" : "Review restaurant order"}</p><h2 id="guest-order-title" className="mb-0 mt-1 text-[28px] font-bold leading-tight tracking-[-0.025em] text-neutral-950">Your order</h2><p className="mb-0 mt-1 truncate text-[15px] text-neutral-400">{activeOutlet.name} · {pointLabel}</p></div></div>
            </div>
              ...
              {cartLines.map(({ item, quantity }) => (
                <div key={item.id} className="flex min-w-0 items-center gap-3 px-4 py-3.5">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mb-0 mt-0.5 text-[13px] font-bold text-emerald-800">{money(item.price * quantity, activeOutlet.currency)}</p>
                  </div>
                  <span className="inline-flex shrink-0 items-center gap-0.5 rounded-md border border-stone-200 bg-stone-50">
                    ...
                    <span className="min-w-5 text-center text-[15px] font-bold text-neutral-900">{quantity}</span>
            ...
            <div className="mt-4 rounded-[10px] border border-stone-200 bg-stone-50/60 p-4">
              <p className="m-0 mb-2 text-[15px] font-bold text-neutral-700">How would you like to pay? <span className="text-red-500">*</span></p>
              {menu.roomChargeAvailable && (
                <div className="grid grid-cols-2 gap-2">
                  <button
                    type="button"
                    onClick={() => setChargeToRoom(false)}
                    className={`min-w-0 rounded-md border px-3 py-3 text-[15px] font-bold transition ${!chargeToRoom ? "border-emerald-600 bg-emerald-50 text-emerald-900 ring-2 ring-emerald-500/10" : "border-neutral-200 bg-white text-neutral-500"}`}
                  >
                    Pay Now
                  </button>
                  <button
              ...
              {!chargeToRoom && (
                <div className={menu.roomChargeAvailable ? "mt-3" : "mt-0"}>
                  <p className="mb-2 mt-0 text-[13px] font-semibold text-neutral-500">Select the method you intend to use. The attendant will confirm it after receiving payment.</p>
                  <div className="grid grid-cols-2 gap-2 sm:grid-cols-3" role="radiogroup" aria-label="Intended payment method">
                    {PAYMENT_METHOD_OPTIONS.map((method) => (
                      <button
                        ...
                        className={`min-h-11 rounded-md border px-2.5 py-2 text-[13px] font-bold transition ${guestPaymentMethod === method.value ? "border-emerald-600 bg-emerald-700 text-white shadow-sm" : "border-stone-200 bg-white text-neutral-600 hover:border-emerald-300 hover:bg-emerald-50"}`}
                      >
                        {method.label}
                      </button>
                    ))}
              ...
              {chargeToRoom && (
                <p className="mb-0 mt-2.5 text-[13px] leading-relaxed text-neutral-400">
            ...
            <label className="mt-4 block min-w-0 max-w-full">
              <span className="mb-1.5 block text-[15px] font-bold text-neutral-600">Note for the {isBarOutlet ? "bar" : "kitchen"} (optional)</span>
              <textarea
                value={note}
            ...
            {placeError && (
              <div className="mt-3 flex items-start gap-2 rounded-xl border border-red-200 bg-red-50 px-3 py-2.5 text-[13px] text-red-700">
            ...
            <p className="mb-0 mt-1.5 text-[13px] leading-5 text-neutral-400">
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-stone-200 border-t-2 border-t-emerald-700 bg-white shadow-[0_12px_34px_rgba(38,35,28,0.07)]" aria-labelledby="bar-shelf-title">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-stone-200 bg-white px-5 py-4">
        <div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-lg bg-emerald-50 text-emerald-800 ring-1 ring-emerald-100"><Wine className="h-[18px] w-[18px]" /></span><div><p className="m-0 text-[11px] font-bold uppercase tracking-[0.16em] text-emerald-700">Shelf register</p><h3 id="bar-shelf-title" className="m-0 mt-0.5 text-base font-bold tracking-[-0.01em] text-neutral-900">Available drinks</h3></div></div>
        <div className="flex items-center gap-1.5 text-[12px] font-bold"><span className="inline-flex items-center gap-1.5 rounded-md bg-emerald-50 px-2.5 py-1.5 text-emerald-800"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />{availableCount} available</span>{items.length - availableCount > 0 && <span className="rounded-md bg-stone-200/70 px-2.5 py-1.5 text-stone-600">{items.length - availableCount} unavailable</span>}</div>
      </header>
      <div className="overflow-x-auto">
        <table className="w-full min-w-[680px] border-collapse text-left">
          <caption className="sr-only">Live bar shelf with drink availability, prices and order controls</caption>
          <thead>
            <tr className="border-b border-stone-200 bg-stone-50/80 text-[11px] font-bold uppercase tracking-[0.14em] text-stone-500">
                ...
                <tr key={item.id} className={`transition ${item.inStock ? "odd:bg-white even:bg-stone-50/35 hover:bg-emerald-50/35" : "bg-stone-50/80 text-stone-400"}`}>
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0"><p className={`m-0 truncate text-[15px] font-bold ${item.inStock ? "text-neutral-900" : "text-stone-500"}`}>{item.name}</p>{item.description && <p className="mb-0 mt-1 max-w-sm truncate text-[12px] leading-4 text-stone-500">{item.description}</p>}</div>
                    </div>
                  </td>
                  <td className="px-4 py-3.5"><span className="inline-flex rounded-md border border-stone-200 bg-white px-2.5 py-1.5 text-[12px] font-bold text-stone-700 shadow-sm">{item.category?.trim() || "General"}</span></td>
                  <td className="px-4 py-3.5"><span className={`inline-flex items-center gap-2 text-[12px] font-bold ${item.inStock ? "text-emerald-700" : "text-stone-400"}`}><span className={`h-2 w-2 rounded-full ${item.inStock ? "bg-emerald-500 shadow-[0_0_0_3px_rgba(16,185,129,0.10)]" : "bg-stone-300"}`} />{item.inStock ? "On shelf" : "Unavailable"}</span></td>
                  <td className={`whitespace-nowrap px-4 py-3.5 text-right text-[15px] font-bold ${item.inStock ? "text-emerald-800" : "text-stone-400"}`}>{money(item.price, currency)}</td>
                  <td className="px-5 py-3.5 text-right">
                    {!item.inStock ? <span className="text-[13px] text-stone-300">—</span> : qty === 0 ? <button type="button" onClick={() => onChangeQty(item.id, 1)} className="inline-flex h-9 min-w-[78px] items-center justify-center gap-1.5 rounded-md border border-emerald-300 bg-emerald-50 px-3 text-[12px] font-bold text-emerald-800 transition hover:border-emerald-400 hover:bg-emerald-100"><Plus className="h-3.5 w-3.5" /> Add</button> : <span className="inline-flex h-9 min-w-[78px] items-center justify-between rounded-md bg-[#073f35] text-white shadow-sm"><button type="button" onClick={() => onChangeQty(item.id, -1)} aria-label={`Remove one ${item.name}`} className="flex h-9 w-8 items-center justify-center border-0 bg-transparent text-white"><Minus className="h-3.5 w-3.5" /></button><span className="min-w-4 text-center text-[12.5px] font-bold">{qty}</span><button type="button" onClick={() => onChangeQty(item.id, 1)} aria-label={`Add one ${item.name}`} className="flex h-9 w-8 items-center justify-center border-0 bg-transparent text-white"><Plus className="h-3.5 w-3.5" /></button></span>}
                  </td>
                </tr>
              );
            })}
      ...
      <p className="m-0 border-t border-stone-100 bg-stone-50/60 px-4 py-2.5 text-[13px] font-semibold leading-5 text-stone-500 sm:hidden">Swipe sideways to view the full shelf register.</p>
    </section>
  );
}
