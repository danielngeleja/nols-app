...
function RestrictionConfirmationCard({ confirmation, busy, roomName, dates, onCancel, onConfirm }: { confirmation: RestrictionConfirmation; busy: boolean; roomName: string; dates: string; onCancel: () => void; onConfirm: () => void }) {
  ...
  return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-neutral-950/40 p-4" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target && !busy) onCancel(); }}>
    <section role="alertdialog" aria-modal="true" aria-labelledby="restriction-confirmation-title" className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl">
      ...
      <footer className="flex items-center justify-end gap-2 border-t border-neutral-100 bg-neutral-50 px-5 py-3">
...
function RestrictionRow({ item, busy, onPatch, onApplyStopSell, onRemove }: { item: any; busy: string | null; onPatch: (body: Record<string, unknown>, note: string) => void; onApplyStopSell: () => void; onRemove: () => void }) {
  ...
  return (
    <article className={`overflow-hidden rounded-xl border ${paused ? "border-dashed border-neutral-300 bg-neutral-50" : "border-neutral-200 bg-white"}`}>
      ...
      <div className="flex flex-wrap items-center gap-2 border-t border-neutral-100 bg-neutral-50/60 px-4 py-2.5">
...
function RatePlanRow({ plan }: { plan: any }) {
  ...
  return <article className="flex min-h-[220px] flex-col overflow-hidden rounded-2xl bg-[#f6f8f8] ring-1 ring-inset ring-neutral-200/80">
    <div className="flex flex-1 flex-col p-4 sm:p-5">
      ...
      <dl className="mt-4 divide-y divide-neutral-200/70 border-t border-neutral-200/80">
    ...
    <div className="border-t border-neutral-200/80 bg-white/80 px-4 py-3 sm:px-5">
...
function ServiceCaseCard({ item, busy, onUpdate }: { item: any; busy: boolean; onUpdate: (status: string) => void }) {
  ...
  return <article className="overflow-hidden rounded-2xl bg-[#f6f8f8] ring-1 ring-inset ring-neutral-200/80">
    <div className="p-4 sm:p-5">
      ...
      <dl className="mt-4 divide-y divide-neutral-200/70 border-y border-neutral-200/80">
    ...
    <footer className="flex flex-wrap items-center gap-2 border-t border-neutral-200/80 bg-white/80 px-4 py-3 sm:px-5">
...
function JourneyTemplateCard({ item }: { item: any }) {
  return <article className="overflow-hidden rounded-2xl bg-[#f6f8f8] ring-1 ring-inset ring-neutral-200/80">
    ...
    <footer className="flex items-center gap-2 border-t border-neutral-200/80 bg-white/80 px-4 py-3 text-[10px] text-neutral-500 sm:px-5"><CheckCircle2 className="h-3.5 w-3.5 text-emerald-600" /><span>{item._count.deliveries} {item._count.deliveries === 1 ? "message" : "messages"} scheduled</span></footer>
  </article>;
}
...
export default function NrmsControlsPage() {
  ...
  return <div id="nrms-controls" className="mx-auto max-w-[1500px] space-y-5 pb-10">
    ...
    <header className="relative overflow-visible rounded-2xl border-2 border-slate-300 bg-[linear-gradient(115deg,#ffffff_0%,#ffffff_55%,#ecfdf5_100%)] shadow-[0_8px_24px_rgba(15,23,42,0.08)]">
      <div className="grid gap-5 px-5 py-4 sm:px-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
        <div className="flex min-w-0 items-center gap-4">
          <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-emerald-800 bg-emerald-950 text-emerald-200 shadow-[0_6px_14px_rgba(6,78,59,0.2)]"><Gauge className="h-5 w-5" /></span>
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2"><p className="m-0 text-[9px] font-bold uppercase tracking-[0.2em] text-emerald-700">NRMS command centre</p><span className="inline-flex max-w-full items-center gap-1.5 rounded-full border border-slate-300 bg-white px-2.5 py-1 text-[9px] font-bold uppercase tracking-[0.08em] text-slate-600 shadow-sm"><span className="h-1.5 w-1.5 shrink-0 rounded-full bg-emerald-500" /><span className="truncate">{selectedProperty?.title || "Property workspace"}</span></span></div>
            <h1 className="mb-0 mt-1.5 text-2xl font-bold tracking-tight text-neutral-950">Hotel controls</h1>
        ...
        <div className="grid overflow-visible rounded-xl border-2 border-slate-300 bg-white/90 p-2 shadow-[0_5px_16px_rgba(15,23,42,0.07)] sm:grid-cols-[minmax(10rem,auto)_auto] sm:items-center">
          <div className="flex min-h-10 items-center gap-2.5 px-2.5 py-1">
            <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${online ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-800"}`}>{online ? <CheckCircle2 className="h-4 w-4" /> : <CloudOff className="h-4 w-4" />}</span>
            <span><span className="block text-[9px] font-bold uppercase tracking-[0.12em] text-neutral-400">Workspace status</span><span className={`mt-0.5 block text-[11px] font-bold ${online ? "text-emerald-800" : "text-amber-900"}`}>{online ? (queued ? `${queued} changes waiting` : "Synced and online") : `${queued} changes saved offline`}</span></span>
          </div>
          <div className="flex flex-wrap items-center gap-2 border-t-2 border-slate-200 pt-2 sm:border-l-2 sm:border-t-0 sm:pl-2 sm:pt-0">
            <ShareBookingButton propertyId={selectedPropertyId} propertyTitle={selectedProperty?.title} />
            <button type="button" onClick={() => void load()} className="inline-flex min-h-10 items-center gap-2 rounded-lg border-2 border-slate-300 bg-white px-3.5 text-xs font-bold text-neutral-700 transition hover:border-emerald-400 hover:bg-emerald-50"><RefreshCw className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} />Refresh</button>
          </div>
        </div>
      </div>
    </header>
            ...
            <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_minmax(9rem,.42fr)]">
              ...
              <label htmlFor="rate-plan-code" className="grid min-w-0 gap-1.5 text-xs font-semibold text-neutral-700">
                ...
                <span className="relative block">
                  <input id="rate-plan-code" aria-invalid={Boolean(duplicateRatePlan)} aria-describedby={duplicateRatePlan ? "rate-plan-code-error" : undefined} autoCapitalize="characters" spellCheck={false} className={`${inputClass} uppercase ${duplicateRatePlan ? "border-red-300 pr-9 focus:border-red-500 focus:ring-red-100" : ""}`} value={rate.code} onChange={(event) => setRate({ ...rate, code: event.target.value })} placeholder="e.g. BAR" />
    ...
    {tab === "guest" && <div className="space-y-5">
      <section className="overflow-hidden rounded-2xl border-2 border-neutral-300 bg-white shadow-[0_10px_30px_rgba(15,23,42,0.08)]">
        <header className="grid gap-4 border-b-2 border-neutral-300 px-5 py-5 sm:px-6 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
        ...
        <div className="grid lg:grid-cols-[minmax(0,1.25fr)_minmax(340px,.75fr)]">
          <div className="p-5 sm:p-6">
            <div className={`flex flex-col gap-4 rounded-xl border-2 p-4 shadow-[0_4px_14px_rgba(15,23,42,0.06)] sm:flex-row sm:items-center sm:justify-between ${guestContact.enabled ? "border-emerald-400 bg-emerald-50/70" : "border-neutral-300 bg-white"}`}>
            ...
            <div className="mt-6">
              ...
              {(() => {
                ...
                return <div className={`mt-4 grid gap-4 rounded-2xl border-2 p-4 shadow-[0_5px_16px_rgba(15,23,42,0.07)] sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center ${connected ? "border-emerald-400 bg-emerald-50/60" : ready ? "border-fuchsia-400 bg-white" : "border-neutral-300 bg-white"}`}>
                  ...
                  {connected ? <button type="button" disabled={busy === "meta-instagram"} onClick={() => void disconnectInstagram()} className="min-h-9 rounded-lg border border-red-300 bg-white px-3 text-[10px] font-bold text-red-700 transition hover:bg-red-50">Disconnect</button> : ready ? <button type="button" disabled={busy === "meta-instagram"} onClick={() => void connectInstagram()} className={buttonClass}>{busy === "meta-instagram" ? <Loader2 className="h-4 w-4 animate-spin" /> : <Instagram className="h-4 w-4" />}Connect account</button> : <span className="inline-flex min-h-9 items-center justify-center gap-2 rounded-lg border border-neutral-300 bg-white px-3 text-[10px] font-bold text-neutral-600 shadow-sm">{checking ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Wrench className="h-3.5 w-3.5" />}{checking ? "Checking" : "Admin setup"}</span>}
                </div>;
              })()}
              {(() => {
          ...
          <aside className="border-t-2 border-neutral-300 bg-neutral-50/80 p-5 sm:p-6 lg:border-l-2 lg:border-t-0">
            ...
            <div className="mt-5 rounded-2xl border-2 border-neutral-300 bg-white p-4 shadow-[0_5px_16px_rgba(15,23,42,0.07)]">
