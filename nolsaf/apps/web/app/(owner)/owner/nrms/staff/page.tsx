...
import { AlertTriangle, Loader2, ShieldCheck, UserPlus, UserX, UsersRound } from "lucide-react";
...
export default function NrmsStaffPage() {
    ...
    try {
      ...
      if (response.data?.needsConfirmation) {
        setNotice(response.data?.emailSent
          ...
          : `Assignment saved as pending, but the invitation email could not be sent to ${assignedEmail}. Assign again to resend it.`);
      } else {
  ...
  return <div className="mx-auto max-w-6xl space-y-4 pb-8">
    <div><p className="m-0 text-[10px] font-bold uppercase tracking-[0.16em] text-emerald-700">Access control</p><h2 className="mb-0 mt-1 text-xl font-bold text-neutral-950">Staff and roles</h2><p className="mb-0 mt-1 text-xs text-neutral-500">Property and outlet memberships do not grant access to the Owner marketplace account.</p></div>
    ...
    <section className="overflow-hidden rounded-3xl border border-neutral-200 bg-white shadow-[0_18px_45px_-36px_rgba(15,23,42,0.5)]">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-neutral-100 bg-neutral-50/70 px-4 py-3.5 sm:px-5">
        <div className="flex min-w-0 items-center gap-3">
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-emerald-100 text-emerald-800"><UserPlus className="h-4 w-4" /></span>
        ...
        <span className="rounded-full border border-neutral-200 bg-white px-2.5 py-1 text-[9px] font-bold uppercase tracking-wide text-neutral-500">Access control</span>
      </div>

      <form onSubmit={(event) => { event.preventDefault(); void assign(); }} className="grid min-w-0 grid-cols-1 gap-3 p-4 sm:p-5 md:grid-cols-2 xl:grid-cols-12 xl:items-end">
        <label className={`min-w-0 text-[10px] font-bold uppercase tracking-wide text-neutral-500 md:col-span-2 ${needsOutlet ? "xl:col-span-5" : "xl:col-span-6"}`}>
          Account email
          <input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="staff@example.com" autoComplete="email" className="mt-1.5 box-border !h-10 w-full min-w-0 rounded-xl border border-neutral-200 bg-neutral-50 px-3 py-0 text-sm font-semibold normal-case tracking-normal text-neutral-900 outline-none transition placeholder:font-normal placeholder:text-neutral-400 focus:border-emerald-500 focus:bg-white focus:ring-2 focus:ring-emerald-500/10" />
        </label>
        <label className="min-w-0 text-[10px] font-bold uppercase tracking-wide text-neutral-500 xl:col-span-3">
          Staff role
          <select value={role} onChange={(event) => { setRole(event.target.value); setOutletId(""); }} className="mt-1.5 box-border !h-10 w-full min-w-0 rounded-xl border border-neutral-200 bg-neutral-50 px-3 py-0 text-sm font-semibold normal-case tracking-normal text-neutral-800 outline-none transition focus:border-emerald-500 focus:bg-white focus:ring-2 focus:ring-emerald-500/10">{ROLES.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select>
        </label>
        {needsOutlet && <label className="min-w-0 text-[10px] font-bold uppercase tracking-wide text-neutral-500 xl:col-span-2">
          Assigned outlet
          <select value={outletId} onChange={(event) => setOutletId(event.target.value ? Number(event.target.value) : "")} className="mt-1.5 box-border !h-10 w-full min-w-0 rounded-xl border border-neutral-200 bg-neutral-50 px-3 py-0 text-sm font-semibold normal-case tracking-normal text-neutral-800 outline-none transition focus:border-emerald-500 focus:bg-white focus:ring-2 focus:ring-emerald-500/10"><option value="">Select outlet</option>{eligibleOutlets.map((outlet) => <option key={outlet.id} value={outlet.id}>{outlet.name}</option>)}</select>
        </label>}
        <button type="submit" disabled={busy || !email.trim() || (needsOutlet && !outletId)} className={`box-border inline-flex !h-10 w-full items-center justify-center gap-2 rounded-xl border-0 bg-[#073c35] px-3 text-xs font-bold text-white shadow-sm transition hover:bg-emerald-800 disabled:cursor-not-allowed disabled:bg-neutral-200 disabled:text-neutral-400 disabled:shadow-none ${needsOutlet ? "md:col-span-2 xl:col-span-2" : "md:col-span-1 xl:col-span-3"}`}>{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <ShieldCheck className="h-4 w-4" />}{busy ? "Assigning..." : "Assign access"}</button>
      </form>
    </section>

    <section className="overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm"><div className="flex items-center justify-between border-b border-neutral-100 px-4 py-3"><div className="flex items-center gap-2"><UsersRound className="h-4 w-4 text-neutral-500" /><h3 className="m-0 text-sm font-bold text-neutral-900">Property team</h3></div><span className="text-[10px] font-bold text-neutral-400">{staff.length} assignments</span></div><div className="divide-y divide-neutral-100">{staff.map((membership) => <div key={membership.id} className="grid min-w-0 gap-2 px-4 py-3 sm:grid-cols-[minmax(0,1fr)_11rem_11rem_8rem_5.5rem] sm:items-center"><div className="min-w-0"><p className="m-0 truncate text-xs font-bold text-neutral-900">{membership.user.fullName || membership.user.name || "Staff member"}</p><p className="mb-0 mt-0.5 truncate text-[10px] text-neutral-400">{membership.user.email || membership.user.phone || `User #${membership.user.id}`}</p></div><span className="text-xs font-semibold text-neutral-600">{ROLES.find((item) => item.value === membership.role)?.label ?? membership.role}</span><span className="text-xs text-neutral-500">{membership.outlet?.name ?? "All property"}</span><span className={`w-fit rounded-full px-2 py-1 text-[9px] font-bold ${membership.status === "ACTIVE" ? "bg-emerald-50 text-emerald-700" : membership.status === "PENDING" ? "bg-amber-50 text-amber-700" : "bg-neutral-100 text-neutral-500"}`}>{membership.status === "PENDING" ? "AWAITING CONFIRMATION" : membership.status === "DISABLED" ? "REVOKED" : membership.status}</span>{membership.status !== "DISABLED" ? <button type="button" onClick={() => openRevoke(membership)} disabled={revokingId !== null} className="inline-flex h-8 w-fit items-center gap-1.5 rounded-lg border border-red-200 bg-white px-2.5 text-[10px] font-bold text-red-600 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-50">{revokingId === membership.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <UserX className="h-3.5 w-3.5" />}Revoke</button> : <span />}</div>)}</div>{staff.length === 0 && <div className="py-14 text-center text-sm text-neutral-400">No NRMS staff assigned yet.</div>}</section>
