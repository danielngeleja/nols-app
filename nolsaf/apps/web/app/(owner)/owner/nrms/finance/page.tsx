...
export default function FinanceControlPage() {
  ...
  const closeAudit = () => action(() => apiClient.post(`/api/owner/nrms/finance/property/${selectedPropertyId}/night-audit/close`, { businessDate }), "Night Audit completed. This date is locked, the next business day is open, and operations can continue.");
  ...
  return <div className="mx-auto max-w-[1500px] space-y-4 pb-10">
    ...
    {tab === "audit" && <div className="grid gap-4 xl:grid-cols-[1.15fr_.85fr]"><section className="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h3 className="m-0 text-base font-bold">Night Audit checklist</h3><p className="mb-0 mt-1 text-xs text-neutral-500">Every blocking item must be cleared. There is no override or bypass.</p></div><CalendarCheck2 className="h-6 w-6 text-emerald-700" /></div><div className="mt-4 space-y-2">{!isCompletedBusinessDate ? <div className="flex gap-3 rounded-xl border border-amber-200 bg-amber-50 p-3"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-amber-700" /><div><p className="m-0 text-xs font-bold text-amber-900">This business date is still operating</p><p className="mb-0 mt-1 text-[10px] text-amber-700">Choose yesterday or an earlier open date. Today cannot be locked while guests and outlets are still operating.</p></div></div> : data?.blockers.length ? data.blockers.map((blocker) => <div key={blocker.code} className="flex gap-3 rounded-xl border border-red-200 bg-red-50 p-3"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-red-600" /><div><p className="m-0 text-xs font-bold text-red-800">{blocker.message}</p><p className="mb-0 mt-1 text-[10px] text-red-600">Resolve this in the related operational workspace, then refresh this control.</p></div></div>) : <div className="flex gap-3 rounded-xl border border-emerald-200 bg-emerald-50 p-4"><CheckCircle2 className="h-5 w-5 text-emerald-700" /><div><p className="m-0 text-xs font-bold text-emerald-800">All closing controls passed</p><p className="mb-0 mt-1 text-[10px] text-emerald-700">NRMS is ready to post balanced entries and lock this completed business date.</p></div></div>}</div><button type="button" onClick={closeAudit} disabled={!canManage || busy || !isCompletedBusinessDate || Boolean(data?.blockers.length) || data?.businessDay.status === "CLOSED"} className="mt-5 inline-flex h-11 w-full items-center justify-center gap-2 rounded-lg border-0 bg-[#073c35] px-4 text-xs font-bold text-white disabled:bg-neutral-200 disabled:text-neutral-400"><LockKeyhole className="h-4 w-4" />{data?.businessDay.status === "CLOSED" ? "Business date closed" : !isCompletedBusinessDate ? "Choose a completed business date" : !canManage ? "Manager approval required to close" : "Post ledger and close business date"}</button></section><section className="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm"><h3 className="m-0 text-sm font-bold">Audit history</h3><div className="mt-3 space-y-2">{data?.businessDay.audits.map((audit) => <div key={audit.id} className="rounded-xl border border-neutral-200 p-3"><div className="flex items-center justify-between gap-3"><strong className="text-xs">{audit.reportNumber}</strong><span className={`rounded-full px-2 py-1 text-[9px] font-bold ${audit.status === "CLOSED" ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}>{audit.status}</span></div><p className="mb-0 mt-1 text-[10px] text-neutral-500">{time(audit.completedAt || audit.startedAt)}</p></div>)}{!data?.businessDay.audits.length && <p className="text-xs text-neutral-400">No Night Audit has been attempted for this date.</p>}</div></section></div>}
