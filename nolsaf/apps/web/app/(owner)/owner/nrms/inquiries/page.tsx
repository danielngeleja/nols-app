...
export default function NrmsGuestInquiriesPage() {
  ...
  return <div className="mx-auto max-w-[1500px] space-y-4 pb-10">
    ...
    <div className="grid gap-3 sm:grid-cols-3">
      {[
        { name: "New", detail: "Awaiting first review", count: counts.new, colour: "bg-violet-50 text-violet-700 ring-violet-300", dot: "bg-violet-500", frame: "border-violet-300 bg-[linear-gradient(135deg,#ffffff_60%,#f5f3ff)]", bar: "bg-violet-500" },
        { name: "Being handled", detail: "Reception is working", count: counts.open, colour: "bg-sky-50 text-sky-700 ring-sky-300", dot: "bg-sky-500", frame: "border-sky-300 bg-[linear-gradient(135deg,#ffffff_60%,#f0f9ff)]", bar: "bg-sky-500" },
        { name: "Waiting for guest", detail: "Reception has replied", count: counts.waiting, colour: "bg-amber-50 text-amber-800 ring-amber-300", dot: "bg-amber-500", frame: "border-amber-300 bg-[linear-gradient(135deg,#ffffff_60%,#fffbeb)]", bar: "bg-amber-500" },
    ...
    {data.reporting && <section className="overflow-hidden rounded-2xl border-2 border-slate-300 bg-white shadow-[0_8px_24px_rgba(15,23,42,0.07)]">
      ...
      {!hasFunnelActivity && <div className="border-b-2 border-slate-300 bg-emerald-50/30 p-3 sm:p-5">
        <div className="relative overflow-hidden rounded-2xl border-2 border-emerald-200 bg-[linear-gradient(135deg,#ffffff_0%,#f0fdf4_100%)] shadow-[0_10px_30px_rgba(6,78,59,0.08)]">
          ...
          <div className="relative p-5 sm:p-7">
            ...
            <div className="mt-6 grid gap-3 md:grid-cols-2 xl:grid-cols-5">
              {connectionHealth.map(({ provider, connection }) => { const connected = connection?.status === "CONNECTED"; return <div key={provider} className={`relative overflow-hidden rounded-xl border-2 p-4 shadow-sm ${connected ? "border-emerald-300 bg-white" : "border-slate-300 bg-slate-50/80"}`}>
                <span className={`absolute inset-y-3 left-0 w-1 rounded-r-full ${connected ? "bg-emerald-500" : "bg-neutral-300"}`} aria-hidden="true" />
                <div className="flex items-center justify-between gap-3"><span className={`flex h-9 w-9 items-center justify-center rounded-lg ${provider === "INSTAGRAM" ? "bg-fuchsia-50 text-fuchsia-700" : "bg-emerald-50 text-emerald-700"}`}><ChannelIcon channel={provider} className="h-4 w-4" /></span><span className={`rounded-full px-2 py-1 text-[8px] font-bold uppercase tracking-wide ${connected ? "bg-emerald-100 text-emerald-800" : "bg-neutral-200 text-neutral-600"}`}>{connected ? "Connected" : "Not connected"}</span></div>
                <p className="mb-0 mt-3 text-xs font-bold text-neutral-950">{provider === "INSTAGRAM" ? "Instagram inbox" : "WhatsApp inbox"}</p><p className="mb-0 mt-1 text-[10px] leading-4 text-neutral-500">{connected ? connection?.displayName || "Ready for guest messages" : "Connect this channel in Hotel controls"}</p>
              </div>; })}
              {[
              ...
              ].map((step) => <div key={step.number} className="rounded-xl border border-emerald-200 bg-white/80 p-4 shadow-sm"><div className="flex items-center gap-2"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-emerald-800 text-[10px] font-black text-white shadow-sm">{step.number}</span><span className="h-px flex-1 bg-emerald-100" /></div><p className="mb-0 mt-3 text-xs font-bold text-neutral-950">{step.title}</p><p className="mb-0 mt-1 text-[10px] leading-4 text-neutral-500">{step.detail}</p></div>)}
            </div>
          </div>
        </div>
      </div>}
      <div className="overflow-x-auto bg-slate-50/70 p-3"><div className="grid min-w-[760px] grid-cols-5 gap-2">{funnelStages.map((stage, index) => <div key={stage.label} className={`relative flex min-h-[124px] flex-col rounded-xl border px-4 py-4 shadow-sm ${["border-violet-300 bg-violet-50/70", "border-sky-300 bg-sky-50/70", "border-cyan-300 bg-cyan-50/70", "border-amber-300 bg-amber-50/70", "border-emerald-300 bg-emerald-50/70"][index]}`}><span className={`mb-3 h-1 w-8 rounded-full ${["bg-violet-500", "bg-sky-500", "bg-cyan-600", "bg-amber-500", "bg-emerald-600"][index]}`} aria-hidden="true" /><p className="m-0 text-[9px] font-bold uppercase tracking-[.12em] text-neutral-600">{stage.label}</p><strong className="mt-2 text-2xl tracking-tight text-neutral-950">{stage.value}</strong><p className="mb-0 mt-auto pt-2 text-[9px] font-semibold text-neutral-500">{index === 0 ? "Starting audience" : stage.rate == null ? "No prior-stage data" : `${stage.rate}% from prior stage`}</p>{index < funnelStages.length - 1 && <ChevronRight className="absolute -right-4 top-1/2 z-10 h-6 w-6 -translate-y-1/2 rounded-full border border-slate-300 bg-white p-1 text-neutral-700 shadow-sm ring-2 ring-slate-50" />}</div>)}</div></div>
    ...
    <section className="grid gap-3 rounded-xl border-2 border-neutral-300 bg-white p-3 shadow-[0_5px_16px_rgba(15,23,42,0.05)] md:grid-cols-[1fr_180px_180px_auto]">
    ...
    <div className="grid min-h-[36rem] items-start gap-4 xl:grid-cols-[23rem_minmax(0,1fr)]">
      <section className="overflow-hidden rounded-2xl border-2 border-neutral-300 bg-white shadow-[0_6px_18px_rgba(15,23,42,0.06)]">
        <div className="flex items-center justify-between border-b-2 border-neutral-300 px-4 py-3"><h2 className="m-0 text-sm font-bold">Inbox</h2><span className="text-[10px] font-semibold text-neutral-400">{data.total} inquiries</span></div>
