    ...
    <div className="flex min-h-[50vh] items-center justify-center px-4 py-6">
      <section id="nrms-freeze-card" role="alert" className="relative w-full max-w-sm overflow-hidden rounded-[24px] border border-red-100 bg-[linear-gradient(180deg,#fff7f7_0%,#ffffff_60%)] p-6 text-center shadow-[0_20px_46px_-26px_rgba(185,28,28,0.4)]">
        ...
        <p className="relative mb-0 mt-2 text-sm leading-6 text-neutral-500">
          {isFrozen
            ? (scope === "enrollment" ? "An administrator paused NRMS access for this owner account until it is reviewed." : "An administrator placed this property in a safety freeze until it is reviewed.")
        ...
        {isFrozen && (referenceCode || reason) && (
          <div className="relative mt-3 rounded-xl border border-red-100 bg-red-50/70 px-3.5 py-2.5 text-left">
            {referenceCode && <><p className="m-0 text-[10px] font-bold uppercase tracking-[0.1em] text-red-500">Appeal reference</p><p className="mb-0 mt-1 break-all font-mono text-sm font-bold text-red-900">{referenceCode}</p></>}
            {reason && <p className={`${referenceCode ? "mt-2" : "mt-0"} mb-0 text-xs leading-5 text-red-800`}>{reason}</p>}
          </div>
        )}
        ...
        <div className="relative mt-2 flex flex-col gap-2">
          {isFrozen && <a href={appealHref} className="inline-flex min-h-10 items-center justify-center rounded-xl bg-[#073c35] px-4 text-sm font-bold text-white no-underline transition hover:bg-[#0a5148] hover:text-white">Contact partners</a>}
