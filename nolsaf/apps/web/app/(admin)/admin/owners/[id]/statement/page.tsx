...
<body>
  <div class="reportPage">
    ...
    <div class="metricGrid">
      ...
      <div class="metricCard${nrms && nrms.accountsCount > 0 ? " metricCardGood" : ""}"><span class="metricLabel">NRMS collected</span><strong>${escapeHtml(cur)} ${escapeHtml(amount(nrms?.collected ?? 0))}</strong><small>${nrms && nrms.accountsCount > 0 ? `Collected across ${escapeHtml(String(nrms.accountsCount))} property account(s). Lifetime NRMS figures; see section 06.` : "No NRMS billing account is recorded for this owner."}</small></div>
      <div class="metricCard"><span class="metricLabel">NRMS billed to date</span><strong>${escapeHtml(cur)} ${escapeHtml(amount(nrms?.billed ?? 0))}</strong><small>${nrms && nrms.accountsCount > 0 ? `${escapeHtml(String(nrms.statementsCount))} statement(s) closed and issued.` : "No NRMS statements have been issued."}</small></div>
      <div class="metricCard${nrms && nrms.outstanding > 0 ? " metricCardWarn" : ""}"><span class="metricLabel">NRMS awaiting collection</span><strong>${escapeHtml(cur)} ${escapeHtml(amount(nrms?.outstanding ?? 0))}</strong><small>${nrms && nrms.accountsCount > 0 ? `${escapeHtml(String(nrms.outstandingCount))} statement(s) remain payable.` : "No NRMS balance is awaiting collection."}</small></div>
