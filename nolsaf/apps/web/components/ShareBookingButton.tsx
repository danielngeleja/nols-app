...
import { Check, Copy, ExternalLink, Facebook, Mail, MessageCircle, Send, Share2 } from "lucide-react";
...
export default function ShareBookingButton({ propertyId, propertyTitle, label = "Share booking page" }: Props) {
  ...
  const copy = async () => { try { await navigator.clipboard.writeText(url); setCopied(true); setTimeout(() => setCopied(false), 1800); } catch { /* clipboard blocked */ } };
  const nativeShare = async () => { try { await navigator.share({ title: propertyTitle || "Direct booking", text: shareText, url }); setOpen(false); } catch { /* user dismissed */ } };
  const openIntent = (href: string) => { window.open(href, "_blank", "noopener,noreferrer"); };

  const targets = [
    { key: "whatsapp", label: "WhatsApp", icon: MessageCircle, className: "bg-[#25D366] text-white hover:opacity-90", href: `https://wa.me/?text=${encodeURIComponent(`${shareText} ${url}`)}` },
    { key: "facebook", label: "Facebook", icon: Facebook, className: "bg-[#1877F2] text-white hover:opacity-90", href: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}` },
    { key: "telegram", label: "Telegram", icon: Send, className: "bg-[#0088cc] text-white hover:opacity-90", href: `https://t.me/share/url?url=${encodeURIComponent(url)}&text=${encodeURIComponent(shareText)}` },
    ...
    <div ref={containerRef} className="relative">
      ...
      {open && (
        <div role="dialog" aria-label="Share direct booking link" className="absolute right-0 z-30 mt-2 w-72 rounded-xl border border-neutral-200 bg-white p-3 shadow-lg">
          ...
          <p className="mb-0 mt-0.5 px-1 text-[11px] leading-4 text-neutral-500">Paste it on Instagram, WhatsApp, Facebook or anywhere guests find you.</p>
